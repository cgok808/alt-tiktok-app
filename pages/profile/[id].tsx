    ...
    <div className='w-full'>
      ...
      <div>
        <div className='flex gap-10 mb-10 mt-10 border-b-2 border-gray-200 bg-white w-full'>
          ...
          <p
            className={`texl-xl font-semibold cursor-pointer mt-2 ${liked}`}
        ...
        <div className='flex gap-6 flex-wrap md:justify-start'>
          {videosList.length >
            0 ? (videosList.map((post: Video, idx: number) => <VideoCard post={post} key={idx}/>)): <NoResults text={`No ${showUserVideos ? "" : "Liked"} Videos Yet`} />}
        </div>
      </div>
    </div>
  );
